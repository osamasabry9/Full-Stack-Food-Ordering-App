...
const socialLinks = [
  ...
  // Add other social links following the same pattern
];

const Footer = async () => {
  const locale = await getCurrentLocal();
  const { home, navbar } = await getTrans(locale);
  const { about } = home;

  const navigationLinks = [
    { label: navbar.about, href: Routes.ABOUT },
    { label: navbar.menu, href: Routes.MENU },
    { label: navbar.contact, href: Routes.CONTACT },
    { label: navbar.login, href: Routes.AUTH },
  ];
